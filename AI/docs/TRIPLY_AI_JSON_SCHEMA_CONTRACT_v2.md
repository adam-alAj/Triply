...
3. **Dataset grounding (FR-AI-002, the non-negotiable rule)** — for each `destination_option`: `destination_name` resolves to exactly one supported `Destination`; every `place_name` (accommodation and daily items) resolves to exactly one active `Place` row scoped to that `destination_id`; the accommodation's resolved place has `place_category_id = ACCOMMODATION` and no daily item resolves to that category. **0% tolerance** — any unresolved name in any returned option fails the whole attempt; invalid options are not silently discarded.
4. **Budget check (deterministic, Backend-computed — no model input)** — for each surviving `destination_option`: sum `(accommodation.nights × Place.reference_price)` + every daily item's `Place.reference_price`. Compare against `Trip.budget_amount`.
   ...
   - **`BUDGET_FIRST`:** all returned options must pass structural and dataset-grounding validation. Backend then checks their deterministic costs in model-returned order and selects the first option within budget. Over-budget options are skipped. At least one fitting option is sufficient for success; if none fit, the attempt fails with `FAILED_VALIDATION` (`ALL_OPTIONS_OVER_BUDGET`). The API returns and persists that one selected itinerary, not a list of fitting options.
5. **Persist** — Backend selects and persists exactly one option: the sole `DESTINATION_FIRST` option, or the first grounded, within-budget `BUDGET_FIRST` option. It sets `Trip.destination_id` to the selected destination and writes `Itinerary`, `ItineraryDay`, and `ItineraryItem` rows in one transaction. Every `estimated_cost` is copied from the resolved `Place.reference_price` (accommodation cost = `nights × reference_price`) — never from the model. `CostEstimate` rows are then computed deterministically from those persisted items (Database Design §15).
6. **On failure** — validation failures set `AIGeneration.status = FAILED_VALIDATION`; upstream Gemini or post-generation cost-persistence failures set `FAILED_ERROR`. `AIGeneration.validation_errors` records the reason. Bounded retries follow Architecture §9; exhausted retries return a clear error and never expose a partial or fabricated itinerary.
...
- **Required provenance:** each generation record persists the schema version used, alongside the model identifier, and the prompt templates declare their target version. Backend writes `AIGeneration.SchemaVersion = 2.0.0` for full and partial generation attempts. Rows created before the provenance migration are marked `unknown`; their historical schema version cannot be reconstructed. Runtime validation uses the configured schema and does not select a schema from the stored generation-row version.
...
| Minimum fitting options for a `BUDGET_FIRST` attempt to count as successful                 | **Resolved — at least one.** Backend selects the first grounded option within budget in model-returned order; zero fitting options fail validation. Additional fitting options are not returned by the API.                                                                                                                    |
| Accommodation slot placement within `days[]` for persistence                                | **Resolved by Backend implementation.** The accommodation row is stored on day 1 at `MORNING`, `order_index = 0`; this is a persistence convention and not part of the model response schema.                                                                                                                                                 |
| Re-introducing `place_category` on `ItineraryItem` for name-collision safety                | **Resolved for current implementation.** Backend detects duplicate active place names within a destination and fails closed; no category field is needed in the model contract today.                                                                                                                                                               |
...
| Persist schema-version provenance on each `AIGeneration`                                    | **Implemented.** `SchemaVersion` is stored for every new full or partial generation attempt; pre-migration rows use `unknown`. See §8 and the Backend migration. |
