...
|---|---|---|---|---|---|---|
| MOB-AUTH-00 | Splash Screen | Onboarding & Auth | Initial app launch screen; handles initial token checks, session restoration, and branding | **P0** | Public | Auto-redirect to Onboarding, Home, or Login |
| MOB-AUTH-01a | Onboarding Screen | Onboarding & Auth | Walkthrough pages introducing Triply features for first-time app users | P2 | Public | Skip, Next, Get Started |
...
| MOB-TRIP-02/03 | Budget & Destination Selection | Trip Creation | Combined screen (`budget_destination_screen.dart`) allowing users to input budget and/or select destination based on planning mode | **P0** | Authenticated | Enter budget/currency, Select destination, Continue |
...
> **Architectural Exception Note (Screen Merging):**  
> In the production implementation, `MOB-TRIP-02` (Budget Input) and `MOB-TRIP-03` (Destination Selection) were merged into a single unified step (`budget_destination_screen.dart`). This decision streamlines the creation wizard UX by handling input collection dynamically depending on the selected `planning_mode` (Destination-First vs. Budget-First) without triggering extra screen navigation transitions.

> **Undocumented Screens Clarification:**  
> `MOB-AUTH-00` (Splash) and `MOB-AUTH-01a` (Onboarding) exist in the codebase but were missing from the initial specification inventory. `MOB-AUTH-00` is documented as **P0** (required for app initialization, token validation, and routing), while `MOB-AUTH-01a` is classified as **P2** (first-time walkthrough onboarding).
...
**J0 — Initial Launch & Session Check:** MOB-AUTH-00 (Splash) → (Checks JWT Session)
* If valid session → MOB-HOME-01
* If first-time user → MOB-AUTH-01a (Onboarding) → MOB-AUTH-02 (Login) / MOB-AUTH-03 (Register)
* If returning unauthenticated user → MOB-AUTH-02 (Login)
...
**J3 — Destination-first trip (SRS journey 2):** MOB-HOME-01 → MOB-TRIP-01 (DESTINATION_FIRST) → MOB-TRIP-02/03 (Budget & Destination Selection) → MOB-TRIP-05 Trip Details → MOB-TRIP-06 Interests → MOB-TRIP-07 Review → MOB-TRIP-08 Generating → MOB-TRIP-09 (Itinerary tab)

**J4 — Budget-first trip (SRS journey 3):** MOB-HOME-01 → MOB-TRIP-01 (BUDGET_FIRST) → MOB-TRIP-02/03 (Budget & Destination Selection) → MOB-TRIP-06 Interests → MOB-TRIP-04 Destination Suggestions → MOB-TRIP-05 Trip Details → MOB-TRIP-07 Review → MOB-TRIP-08 → MOB-TRIP-09
...
**J8 — Unsupported destination (SRS journey 7):** MOB-TRIP-02/03 → "destination not supported" state on the same screen → suggest supported list — no new screen
...
│
├── INITIALIZATION
│   └── MOB-AUTH-00 (Splash Screen) ───→ (Check JWT / First Time)
│            ├── First Time User ───────→ MOB-AUTH-01a (Onboarding)
│            ├── Unauthenticated ───────→ AUTH STACK
│            └── Authenticated ─────────→ MAIN (Authenticated Bottom Nav)
│
...
MOB-TRIP-01 Mode → MOB-TRIP-02/03 Budget & Destination →
...
↓
[2] BUDGET & DESTINATION SELECTION (MOB-TRIP-02/03)
Destination-first: pick from supported list      REQUIRED · Trip.destination_id · editable until generation
Budget-first: amount + currency                  REQUIRED in budget-first (Trip.budget_amount);
...
|---|---|---|---|---|---|
| Splash Screen | logo animation / spinner | — | initialization error | → Onboarding / Auth / Home | token verification |
| Onboarding | — | — | — | walkthrough active | skip / finish actions |
...
| Budget & Destination Selection | skeleton | — | fetch error + retry | list, search results | **"Destination not supported" inline notice (SRS journey 7)** |
...
**Initialization & Authentication:** MOB-AUTH-00 Splash · MOB-AUTH-02 Login · MOB-AUTH-03 Register

**Home:** MOB-HOME-01 Home

**Trip Planning:** MOB-TRIP-01 Mode · MOB-TRIP-02/03 Budget & Destination Selection · MOB-TRIP-04 Destination Suggestions · MOB-TRIP-05 Trip Details · MOB-TRIP-06 Interests · MOB-TRIP-07 Review & Confirm · MOB-TRIP-08 Generating
...
| MOB-AUTH-01a Onboarding Screen | First-time walkthrough, non-blocking (P2) |
...
|---|---|---|
| Splash Screen | App Boot, JWT Session Check | J0 |
...
| Budget & Destination Selection | FR-TRIP-001, FR-TRIP-002, D5 supported list, journey 7 | J3, J4, J8 |
...
**Design these (P0 first):** the 11 MVP screens in Section 9, in the order of journeys J0–J9. Primary flows: **J3 (destination-first)** and **J4 (budget-first)** — these two are the product; everything else supports them.
